...
pca_plot = ggarrange(pca_digesitonBatch_batch_AgingMice + 
                       ggtitle(NULL)+
...
panel_bottom2 = ggarrange(raw_norm_comparison+
                         ...
                         p_span+
                           theme(plot.margin = margin(t=1, l=0.5, 
                                                      r=.25, b=.5, unit = "cm")), ncol = 2, align = 'v', font.label = list(size=24), 
                         widths = c(.9, 1.1), labels = c('C','D'))

supp_fig3 = ggarrange(panel_A_B, panel_bottom2, nrow =2, heights = c(2.5, 2))
ggsave(supp_fig3, filename = 'plots_AgingMice/SuppFig3_pipeline_extras.pdf',
       width = 20, height = 12)
